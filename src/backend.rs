use crate::define::mir::{
    Block as MirBlock, Module as MirModule, StmtKind as MirStmtKind, SymbolKind as MirSymbolKind,
...
impl<'ctx> LlvmIrCodegenContext<'ctx> {
            ...
            match &symbol.as_ref().borrow().kind {
                MirSymbolKind::Def(builtin, func) => {
                    if *builtin {
                        ...
                        let ll_ty =
                            self.codegen_type(func.borrow_mut().fetch_type(symbol_table.clone()))?;

                        let ll_fn_ty = match ll_ty {
                            AnyTypeEnum::FunctionType(ty) => ty,
                            _ => return Err("Invalid function type".to_string()),
                        };

                        let ll_func = self.module.add_function(name.as_str(), ll_fn_ty, None);

                        if let MirValueKind::Fn(ref func) = func.as_ref().borrow().kind {
                            if let MirValueKind::Block(ref block) =
                                func.body.as_ref().unwrap().as_ref().borrow().kind
                            {
                                let label = block.label.clone();
                                let ll_basic_block =
                                    self.context.append_basic_block(ll_func, label.as_str());

                                self.builder.position_at_end(ll_basic_block);
                                self.codegen_block(block)?;
                            } else {
                                return Err("Invalid internal value for function".to_string());
                            }
                        } else {
                            return Err("Invalid value for `def` item.".to_string());
                        };
                    }
                }
    ...
    fn codegen_type(&mut self, mir_value: Ptr<MirValue>) -> Result<AnyTypeEnum<'ctx>, String> {
        match &mir_value.as_ref().borrow().kind {
    ...
    fn codegen_basic_value(
        ...
        match &mir_value.as_ref().borrow().kind {
            MirValueKind::Lit(radix, text) => {
                ...
                match ll_ty {
                    IntType(ty) => Ok(ty
                        .const_int_from_string(text.as_str(), ll_radix)
                        .unwrap()
                        .into()),
            ...
            match &stmt.as_ref().borrow().kind {
                MirStmtKind::Return(value) => {
                    if let Some(symbol) = value {
                        let ll_value =
                            self.codegen_basic_value(match &symbol.as_ref().borrow().kind {
                        ...
                        self.builder.build_return(match &ll_value {
