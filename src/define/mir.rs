...
impl Value {
    pub fn new(kind: ValueKind, ty: Option<Ptr<Value>>, span: Span) -> Ptr<Value> {
                ...
                ValueKind::Fn(func) => {
                    ...
                    for param in &func.params {
                        params.push(FnParam::new(
                            param.name.clone(),
                            param.ty.borrow_mut().fetch_type(symbol_table.clone()),
                            param.implicit,
                        ));
                    }
                    let ret_ty = func.ret_ty.clone();
                    let ty = Value::mk_fn_ty(params, ret_ty.unwrap());
...
impl Symbol {
    ...
    pub fn mk_def(name: String, builtin: bool, func: Ptr<Value>) -> Ptr<Symbol> {
...
impl SymbolTable {
    ...
    pub fn register_def(&mut self, name: String, builtin: bool, func: Ptr<Value>) -> Ptr<Symbol> {
...
impl GlobalContext {
    ...
    pub fn fetch_block_label(&mut self) -> String {
        let label = format!("__BLOCK_{}", self.block_lbl_cnt);
        self.block_lbl_cnt += 1;
        return label;
    }
    ...
    pub fn fetch_temprary_name(&mut self) -> String {
        let name = format!("__TEMP_{}", self.temporary_cnt);
        self.temporary_cnt += 1;
        return name;
    }

    pub fn fetach_lambda_fn_name(&mut self) -> String {
        let name = format!("__LAMBDA_{}", self.lambda_fn_cnt);
        self.lambda_fn_cnt += 1;
        return name;
    }
}
...
impl ast::Block {
    pub fn to_mir(&self, symbol_table: Ptr<SymbolTable>, global_ctx: &mut GlobalContext) -> Block {
...
impl ast::Item {
    pub fn to_mir(&self, symbol_table: Ptr<SymbolTable>, global_ctx: &mut GlobalContext) {
        ...
        match &self.kind {
            AstItemKind::Def(def) => {
                ...
                let block = if let Some(expr) = &func.expr {
                    Some(match &expr.kind {
                        ...
                        _ => {
                            ...
                            let mut block = self::Block::new(label, func_symbol_table.clone());
                ...
                let ret_ty = if let Some(ref expr) = func.ret_ty {
                    Some(expr.to_mir(func_symbol_table.clone(), global_ctx))
                } else {
                    None
                };

                let value = Value::mk_fn(
                    name.clone(),
                    params,
                    ret_ty,
                    block,
                ...
                let _symbol = symbol_table
                    .borrow_mut()
                    .register_def(name.clone(), builtin, value);
            }
            _ => unimplemented!(),
        }
    }
